...
class TestS7commServer:
    ...
    def test_register_all_data_areas(self, mock_server_class: Mock) -> None:
        ...
        custom_size = 1000
        server = S7commServer(ip="127.0.0.1", port=5102, size=custom_size)
        
        # Verify register_area was called 6 times (DB, PA, PE, MK, TM, CT)
        assert mock_server_instance.register_area.call_count == 6
        
        # Verify each area was registered
        expected_calls = [
            call(snap7.SrvArea.DB, 1, MagicMock()),  # DB1
            call(snap7.SrvArea.PA, 0, MagicMock()),  # PA
            call(snap7.SrvArea.PE, 0, MagicMock()),  # PE
            call(snap7.SrvArea.MK, 0, MagicMock()),  # MK
            call(snap7.SrvArea.TM, 0, MagicMock()),  # TM
            call(snap7.SrvArea.CT, 0, MagicMock()),  # CT
        ]
    ...
    def test_bytearray_initialization(self, mock_server_class: Mock) -> None:
        ...
        server = S7commServer(ip="127.0.0.1", port=5102, size=100)
